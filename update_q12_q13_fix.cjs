...
try {
    ...
    let db = JSON.parse(data); // db is the whole object
    ...
    if (q12Index !== -1) {
        ...
        // Clean up excessive escapes and newlines
        // Replace \\( with ( and \\) with )
        a12 = a12.replace(/\\\\\\(/g, '(').replace(/\\\\\\)/g, ')');
        // If there were simple \\( check for those too
        a12 = a12.replace(/\\\\\\(/g, '(').replace(/\\\\\\)/g, ')');
        // Remove \\ before hyphens or newlines if any
        a12 = a12.replace(/\\\\-/g, '-');

        // Ensure nice Markdown for headers
        // Replace "##\n\n12." with "## 12."
        a12 = a12.replace(/##\s*\n+\s*12\./, '## 12.');
    ...
    if (q13Index !== -1) {
        // User text with standard Markdown formatting (bullets *, numers 1.)
